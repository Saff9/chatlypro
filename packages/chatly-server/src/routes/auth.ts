...
export async function authRoutes(fastify: FastifyInstance, options: FastifyPluginOptions) {
  ...
  fastify.post('/register', async (request, reply) => {
    ...
    try {
      ...
      await pool.query(
        ...
        [email, verificationCode, verificationExpiry]
      );
      ...
      const token = jwt.sign({ userId: newUserId, username: cleanUsername, emailVerified: false }, JWT_SECRET);
      ...
      // In-Memory Fallback
      const userExists = inMemoryUsers.some(u => u.email === email || u.username === cleanUsername);
      if (userExists) {
      ...
      // Save code to memory
      inMemoryEmailVerifications.set(email, {
      ...
      const token = jwt.sign({ userId: newId, username: cleanUsername, emailVerified: false }, JWT_SECRET);
  ...
  fastify.post('/verify-email', async (request, reply) => {
    ...
    try {
      ...
      if (verifyRes.rows.length > 0) {
        // Update user status
        await pool.query('UPDATE users SET email_verified = TRUE WHERE email_encrypted = $1', [email]);
        await pool.query('DELETE FROM email_verifications WHERE email = $1', [email]);

        const userRes = await pool.query('SELECT id, username FROM users WHERE email_encrypted = $1', [email]);
        const user = userRes.rows[0];

        const token = jwt.sign({ userId: user.id, username: user.username, emailVerified: true }, JWT_SECRET);
    ...
    const otpRecord = inMemoryEmailVerifications.get(email);
    if (otpRecord && otpRecord.code === code && Date.now() < otpRecord.expiresAt) {
      ...
      if (memoryUser) {
        ...
        inMemoryEmailVerifications.delete(email);

        const token = jwt.sign({ userId: memoryUser.id, username: memoryUser.username, emailVerified: true }, JWT_SECRET);
  ...
  fastify.post('/resend-verification', async (request, reply) => {
    ...
    try {
      const userRes = await pool.query('SELECT id FROM users WHERE email_encrypted = $1', [email]);
    ...
    if (memoryUser) {
      ...
      return reply.send({ success: true, message: 'Verification code sent in memory mode.' });
    }
  ...
  fastify.post('/login', async (request, reply) => {
    ...
    try {
      // Postgres check
      const result = await pool.query('SELECT * FROM users WHERE email_encrypted = $1', [email]);
  ...
  fastify.post('/verify-2fa', async (request, reply) => {
    ...
    try {
      const otpRes = await pool.query(
        'SELECT * FROM two_factor_temp WHERE user_id = $1 AND code = $2 AND expires_at > NOW()',
        [userId, code]
      );
      if (otpRes.rows.length > 0) {
        await pool.query('DELETE FROM two_factor_temp WHERE user_id = $1', [userId]);

        const token = jwt.sign({ userId, username, emailVerified: true }, JWT_SECRET);
